...
protected:
    ...
    // For UT Walk
    void calculateAngles();//预测角度
    void preProcessJoints();//joints关节
     ...
     * walk direction and rotation using the currently implemented实现
    ...
    void getTargetDistanceAndAngle(const VecPosition &target, double &distance, double &angle);//****用于获取目标距离和角度
    ...
    SkillType playOn();
    SkillType kickOffLeft();
    SkillType kickOffRight();
    SkillType goalKickLeft();
    SkillType goalKickRight();
    SkillType kickInLeft();
    SkillType kickInRight();
    SkillType cornerKickRight();
    SkillType cornerKickLeft();
    //SkillType offsideLeft();
    //SkillType offsideRight();
    //SkillType Golie_Num1_Move();
    //SkillType Golie_Walk_Defend();
    //SkillType AttackNumStrategy(bool isoppclosetoball);
    //SkillType DenfendEmergencyStrategy(bool isDenfendEmergency);
    //SkillType facingballkick();
    

    // 6.20 add
    // for test only
    SkillType testPlayMode();
    // 获得离球最近的队友的编号
    int nearest2BallTM();
    // 获得离球最近的敌方的编号
    int nearest2BallOP();
    ...
    bool ballInOpponent();
    // 获得队友的位置信息
    vector<VecPosition> getTeammate(vector<int>& );
    // 获得敌方的位置信息
    vector<VecPosition> getDangerOP(vector<int>&);
    // 获得离自己最近的敌方的距离
...
public:
    ...
    //VecPosition getFormationPos(std::vector<int> players, std::vector<VecPosition> formationPositions, WorldModel* worldModel) ;
